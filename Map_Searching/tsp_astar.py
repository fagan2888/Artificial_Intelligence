        ...
        while(queue):
            ...
            for child in Tree(node1.city_name).get_Children_name():
                if child not in path:
                    not_visit = [item for item in graph.viewkeys() if item not in path]
                    for n in not_visit:
                        hn.append(coordist[n+child])
                    nearest_dist, inde = self.find_min_ind(hn)
                    cost_list.append((child, dist[child + node1.city_name]+nearest_dist))
