...
from coor_graph import coorGraph
...
class astar_Search:
    ...
    def a_starSearch(self):
        ...
        goal = []
        num_node = 0
        codist0 = coordist[self.start_city.city_name + self.end_city.city_name]
        queue.append((self.start_city.city_name, 0, codist0))
        path[self.start_city.city_name] = (self.start_city.city_name)
        while(queue):
            index = queue[0][1] + queue[0][2]
            order = 0
            for i in range(len(queue)):
                if (queue[i][1] + queue[i][2])< index:
                    index = queue[i][1] + queue[i][2]
                    order = i
            name, cost, heur = queue.pop(order)
            num_node += 1
            if name == self.end_city.city_name:
                if goal == []:
                    goal = [cost, path[name]]
                elif goal != [] and any([cost + heur > queue[i][1] + queue[i][2] for i in range(len(queue))]):
                    continue
                elif all([cost + heur < queue[i][1] + queue[i][2] for i in range(len(queue))]):
                        ...
                        if child in path.viewkeys():
                            ...
                            for i in range(len(queue)):
                                if queue[i][0] == child:
                                    ...
                                    to_end = queue[i][2]
                                    index1 = i
                            if old_cost + to_end > dist[child + name] + cost + heur:
                                del queue[index1]
                                new_cost = dist[child + name] + cost
                                new_heur = heur
                                queue.append((child, new_cost, new_heur))
                                tem_tuple = path[name]
                                path[child] = ((tem_tuple) + (' -> ') + (child))
                        elif child not in path.viewkeys():
                            codist = coordist[child + self.end_city.city_name]
                            total_cost = dist[child + name] + cost
                            queue.append((child, total_cost, codist))
                            tem_tuple = path[name]
                            path[child] = ((tem_tuple) + (' -> ') + (child))
        return goal,num_node
